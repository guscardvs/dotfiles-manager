...
class DefaultLinker:
    ...
    def link(self, dotfile: Dotfile) -> None:
        ...
        print_colored(f"Linked {dotfile.name} to {out_location}", "green")
    ...
    def _link_direct(self, dotfile: Dotfile) -> str:
        ...
        if out_location.exists():
            if (
                ...
                and out_location.resolve() == floc.resolve()
            ):
                print_colored(
    ...
    def save_from_system(self, dotfiles: Collection[Path]):
        ...
        """
        print_colored(
            "Updating manifest with current system's dotfiles...", "yellow"
        )
        for file in dotfiles:
            print_colored(f"Checking dotfile: {file}", "cyan")
        ...
        print_colored(
            "Manifest updated with current system's dotfiles.", "green"
        )
    ...
    def unlink(self, dotfile: Dotfile) -> None:
        ...
        print_colored(
            f"Unlinking {dotfile.name} from {out_location}", "yellow"
        )
        if out_location.is_dir():
            print_colored(
                f"Removing directory {out_location} before unlinking.",
                "red",
            )
            ...
            out_location.unlink()
        print_colored(f"Unlinked {dotfile.name} from {out_location}", "green")
    ...
    def sync_orphaned(self, orphaned_files: list[Path]) -> None:
        ...
        if not orphaned_files:
            print_colored("No unmanaged files to sync.", "green")
            return
        print_colored("Syncing unmanaged files and directories...", "yellow")
        for file in orphaned_files:
            likely_location = Path.home() / file.relative_to(
                self.manifest.root
            )
            dotfile = Dotfile(
                name=file.name,
            ...
            if likely_location.exists() and (
                ...
                print_colored(
                    f"Dotfile {dotfile.name} already linked.", "green"
                )
            else:
                self.link(dotfile)
            self.manifest.dotfiles.append(dotfile)
    ...
    def remove_ghost_refs(self) -> int:
        ...
        """
        print_colored(
            "Removing ghost references from the manifest...", "yellow"
        )
        ...
        if removed_count > 0:
            ...
            for dotfile in missing:
                print_colored(
                    f"- {dotfile.name} ({dotfile.dflocation})", "red"
                )
    ...
    def make_backups(self, files: list[Path]) -> list[tuple[Path, Path]]:
        ...
        for file in files:
            if not file.exists():
                print_colored(
                    f"File {file} does not exist. Skipping backup.", "red"
                )
                continue
            ...
            print_colored(f"Backed up {file} to {backup_file}", "green")
        return backups
    ...
    def restore_backups(self, backups: list[tuple[Path, Path]]) -> None:
        ...
        for original, backup in backups:
            if not backup.exists():
                print_colored(
                    f"Backup {backup} does not exist. Skipping restore.",
                    "red",
                )
                continue
            if original.exists():
                print_colored(
                    f"Original file {original} exists. Overwriting it.",
                    "yellow",
                )
            if backup.is_file():
            ...
            print_colored(f"Restored {original} from {backup}", "green")
    ...
    def delete_backups(self, backups: list[tuple[Path, Path]]) -> None:
        ...
        for original, backup in backups:
            if not backup.exists():
                print_colored(
                    f"Backup {backup} does not exist. Skipping deletion.",
                    "red",
                )
                continue
            if backup.is_file():
                ...
                shutil.rmtree(backup, ignore_errors=True)
            print_colored(f"Deleted backup {backup} for {original}", "green")
    ...
    def persist_links(self) -> None:
        ...
        """
        print_colored("Persisting links to the manifest...", "yellow")
        ...
        print_colored("Links persisted to the manifest.", "green")
